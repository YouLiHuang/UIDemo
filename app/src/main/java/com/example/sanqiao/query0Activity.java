...
public class query0Activity extends AppCompatActivity implements View.OnClickListener {
    ...
    private List<Weldinginfo> weldinginfoList;//不同焊丝直径对应的参数组
    private int wireDiameter_count,page_index=1;//初始为第一页
    ...
    private void showLvData( String wireDiameter,List<oneGroupInfo> pramlist) {
        if (lvDataAdapter == null) {
            ...
            // 更新数据时的操作
            if(pramlist==null) CommonUtils.showDlgMsg(query0Activity.this, "数据库当中没有相关信息");
            else {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if(Math.abs(velocityX) > Math.abs(velocityY)) {
                if (velocityX > 0) {
                    ...
                    {
                        page_index++;
                        CommonUtils.showDlgMsg(query0Activity.this, "第一页");
                    }
                    else {
                        wireDiameter = weldinginfoList.get(page_index-1).getWireDiameter();//焊丝直径
                        pramlist = weldinginfoList.get(page_index-1).getWeldingList();//这个直径下的参数列表
                    ...
                    {
                        page_index--;
                        CommonUtils.showDlgMsg(query0Activity.this, "最后一页");
                    }
                    else {
                        wireDiameter = weldinginfoList.get(page_index-1).getWireDiameter();//焊丝直径
                        pramlist = weldinginfoList.get(page_index-1).getWeldingList();//这个直径下的参数列表
